...
# TFTP에서 정의한 전송모드. 본 코드에서는 octet 사용
...
def send_wrq(filename, mode):
    # TFTP 형식에 따라 struct.pack을 사용하여 WRQ 메시지 패킹
...
def send_ack(seq_num, server):
    # TFTP 형식에 따라 struct.pack을 사용하여 ACK 메시지 패킹
    format = f'>hh'
...
def receive_data(filename):
    ...
    # 수신 된 데이터를 쓸 파일 열기(이진 쓰기 모드)
    file = open(filename, 'wb')
    while True:
        # 서버는 데이터 전송을 위해 새로운 포트 생성
        # 데이터와 서버의 새 소켓 주소 받음
        ...
        if opcode == OPCODE['DATA']:
            ...
            # 블록 번호가 기대한 번호와 다르면 실행(데이터 전송 중 오류)
            else:
        ...
        # 메시지 유형이 DATA, ERROR가 모두 아닐경우 실행
        else:
            break

        # 파일 블록이 블록 크기보다 작을 때(마지막 데이터일 떄) 파일 닫기
